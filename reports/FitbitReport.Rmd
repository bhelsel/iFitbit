...
<hr class="heavy">
<h2> Participant ID: `r paste0("#", id)` </h2>
<h4> This report was created for the ChefBoyID Cooking Pilot Study </h4>
<h4> by Brian Helsel on `r format(Sys.Date(), "%B %d, %Y")` at  `r format(Sys.time(), "%H:%M:%S")` </h4>
<center><img src="/Users/bhelsel/Desktop/Fitbit API/reports/images/fitbit.logo.png" alt="Fitbit logo"/></center>
...
exerciseLog <- DBI::dbReadTable(con, "exerciseLog")
...
batteryColor <- case_when(as.numeric(device$batteryLevel) >= 50 ~ "#39ff14",
...
footprint.img <-  magick::image_read("/Users/bhelsel/Desktop/Fitbit API/reports/images/footprint.jpeg")
activities <- mutate_if(activities, is.character, as.numeric)
...
findActivityAverages <- function(days){
  ...
  findMean <- function(variable, days) {
    activities %>% 
      arrange(desc(date)) %>%
      top_n(days, date) %>%
      mutate(mvpa = minutesFairlyActive + minutesVeryActive,
             pa = minutesLightlyActive + minutesFairlyActive + minutesVeryActive) %>%
      summarise_at(variable, .funs = function(x) round(mean(x, na.rm = TRUE))) %>%
      as_vector() %>%
      unname()
  }
...
p.steps <- activities %>%
  mutate(Category = ifelse(steps >= mean(steps, na.rm = TRUE), "Above Mean", "Below Mean")) %>%
  rename("Date" = "date", "Steps" = "steps") %>%
  ggplot(aes(y = Steps)) +
...
p.steps.plotly <- ggplotly(p.steps, dynamicTicks = TRUE) %>%
  layout(showlegend = FALSE, xaxis = list(rangeslider = list(visible = TRUE),
                                          rangeselector = list(
                                            buttons = list(
...
p.mvpa.plotly <- ggplotly(p.mvpa, dynamicTicks = TRUE) %>%
  layout(showlegend = FALSE, xaxis = list(rangeslider = list(visible = TRUE),
                                          rangeselector = list(
                                            buttons = list(
                                              list(count=7, label="1w", step="day", stepmode="backward"),
...
subplot(p.steps.plotly, p.mvpa.plotly) %>% layout(annotations = annotations)
...
activities %>%
  ...
  select(date, steps, minutesSedentary, minutesLightlyActive, minutesFairlyActive, minutesVeryActive,
         activityCalories, calories, distance, elevation, floors) %>%
  reactable(columns = list(
            date = colDef(name = "Date", filterable = TRUE, align = "center", minWidth = 125),
            steps = colDef(name = "Steps", align = "center"),
            minutesSedentary = colDef(name = "Sedentary", align = "center", minWidth = 110),
            minutesLightlyActive = colDef(name = "Lightly Active", align = "center"),
            minutesFairlyActive = colDef(name = "Fairly Active", align = "center"),
            minutesVeryActive = colDef(name = "Very Active", align = "center"),
            activityCalories = colDef(name = "Activity", align = "center"),
            calories = colDef(name = "Daily", align = "center"), 
            distance = colDef(name = "Distance", align = "center"), 
            elevation = colDef(name = "Elevation", align = "center"),
            floors = colDef(name = "Floors", align = "center")
            ),
          columnGroups = list(
            colGroup(name="Physical Activity", 
                     columns = c( "steps", "minutesSedentary", "minutesLightlyActive", 
                                  "minutesFairlyActive", "minutesVeryActive"), 
                     align = "center"),
            colGroup(name="Calories", 
...
heart.img <-  magick::image_read("/Users/bhelsel/Desktop/Fitbit API/reports/images/heart.jpeg")

# Most recent session is equal to 1
exerciseLogplot <- function(session=1){
  ggplot() +
  geom_rect(aes(xmin = 0, xmax = 500, ymin = 0, ymax = 500), fill = "white", color = "black", size = 2) +
  annotation_raster(heart.img, xmin = 300, xmax = 450, ymin = 150, ymax = 400) +
  annotate("text", x = 475, y = 50, label = format(as.Date(exerciseLog$date[session]), "%B %d, %Y"), hjust = 1, size = 12, fontface = "bold") +
  annotate("text", x = 10, y = 450, label = exerciseLog$type[session], hjust = 0, size = 12, fontface = "bold") +
  annotate("text", x = 10, y = 350, label = paste(round(as.numeric(exerciseLog$duration[session])), "minutes"), hjust = 0, size = 12) +
  annotate("text", x = 10, y = 250, label = paste(exerciseLog$calories[session], "kcals"), hjust = 0, size = 12) +
  annotate("text", x = 10, y = 150, label = paste(exerciseLog$hr[session], "avg bpm"), hjust = 0, size = 12) +
  theme(panel.background = element_blank(), axis.text = element_blank(),
        axis.ticks = element_blank(), axis.title = element_blank())
}
...
exerciseLog %>%
  arrange(desc(date)) %>%
  select(date, time, type, duration, steps, calories, hr) %>%
  reactable(columns = list(
            date = colDef(name = "Date", filterable = TRUE, align = "center"),
            time = colDef(name = "Time", align = "center"),
            type = colDef(name = "Type", align = "center"),
            duration = colDef(name = "Duration (minutes)", align = "center"),
            steps = colDef(name = "Steps", align = "center"),
            calories = colDef(name = "Calories", align = "center"),
            hr = colDef(name = "Heart Rate (bpm)", align = "center")
            ),
          showPageSizeOptions = TRUE,
...
pa.types1 <- heart %>%
  mutate(pa = near.maximal + vigorous + moderate + light + very.light) %>%
  select(date, nonwear, sleep, sedentary, pa) %>%
  pivot_longer(cols = nonwear:pa) %>%
  mutate(Type = factor(name, levels = c("pa", "sedentary", "nonwear", "sleep"), 
                            labels = c("Physical Activity", "Sedentary", "Nonwear", "Sleep"))) %>%
  rename("Date" = "date", "Minutes" = "value") %>%
  ggplot(aes(x = Date)) +
...
pa.types1.plotly <- ggplotly(pa.types1, dynamicTicks = TRUE) %>%
  layout(showlegend = FALSE, xaxis = list(rangeslider = list(visible = TRUE),
                                          rangeselector = list(
                                            buttons = list(
                                              list(count=7, label="1w", step="day", stepmode="backward"),
...
pa.types2 <- heart %>%
  ...
  pivot_longer(cols = very.light:mvpa) %>%
  mutate(Intensity = factor(name, levels = c("mvpa", "light", "very.light"), 
...
pa.types2.plotly <- ggplotly(pa.types2, dynamicTicks = TRUE) %>%
  layout(showlegend = FALSE, xaxis = list(rangeslider = list(visible = TRUE),
                                          rangeselector = list(
                                            buttons = list(
                                              list(count=7, label="1w", step="day", stepmode="backward"),
...
subplot(pa.types1.plotly, pa.types2.plotly) %>% layout(annotations = annotations)
