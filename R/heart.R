...
get.fitbit.heart.intraday <- function(directory, id, start.date = "", end.date = "", detail.level = "1min"){
  ...
  if(DBI::dbExistsTable(con, "heart")){
    ...
    date.range <- seq(as.Date(start.date), as.Date(end.date), 1)
    new.start.date <- date.range[!date.range %in% as.Date(currentData$date)]

    if(length(new.start.date)!=0){
      start.date <- min(as.character(new.start.date))
      currentData <- currentData[currentData$date < start.date, ]
    }
    
    if(length(new.start.date)==0){
      start.date <- as.character(max(as.Date(currentData$date)))
      currentData <- currentData[currentData$date < start.date, ]
    }
  }
